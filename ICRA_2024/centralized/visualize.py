...
class Animation:
  def __init__(self, map, schedule):
    ...
    for io in map["inflated_obstacles"]:
      if io not in map["map"]["obstacles"]:
        x, y = io[0], io[1]
        self.patches.append(Rectangle((x - 0.5, y - 0.5), 1, 1, facecolor='red', edgecolor='green'))
    ...
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      name = d["name"]
      self.agents[name] = Circle((d["start"][0], d["start"][1]), diameter/2 , facecolor=Colors[0], edgecolor='black')
