...
class Prb_check_n_cluster:
    ...
    def get_scenario_clusters(self, risk_score, prob_collision): 
        ...
        K_max = self.K_max
        obs_list  = list(risk_score.keys())
        # 2) Pick the top K_max-1 anchors
        n_anchors = max(0, K_max - 1)
        anchors = [(i, j) for i, j, _ in flat[:n_anchors]]
        obs_anchors  = [next(((i,j) for i,j,_ in flat if i == obs_idx), None) for obs_idx in obs_list] 
        
        if len(obs_anchors) > n_anchors:
            obs_anchors = obs_anchors[:n_anchors]
        elif len(obs_anchors) < n_anchors:
            extra_anchors = []
            ctr = 1
            while len(extra_anchors)<n_anchors - len(obs_anchors):
                if flat[ctr] not in obs_anchors:
                    extra_anchors.append(flat[ctr])
                ctr+=1
            obs_anchors.extend(extra_anchors)
        # 3) Enumerate ALL scenarios as tuples of length N_obs
        all_scenarios = self.all_scenarios
        # anchors = obs_anchors
        # obs_anchors = set(obs_anchors)
        # print(obs_anchors)
        # 4) Build one cluster per anchor
        def _contains_anchor(scene_mode, k):
...
class Simulator():
    ...
    def _get_idm_params(self, v, cl, v_, cl_, verbose = False):
        ...
        for i, (vh, clh) in enumerate(zip(v_,cl_)):
            ...
            if self.sinks[cl]==self.sinks[clh]:
                ...
                v_pos = self.routes[cl](v[0]-.0)[:2].reshape((-1,1))
    ...
    def get_deviation(self, s, ey, epsi):
        x_dev = -ey*np.sin(self.routes[self.ev.cl](s)[2])
        y_dev = ey*np.cos(self.routes[self.ev.cl](s)[2])
        psi_dev = np.clip(epsi, -0.15, 0.15)
        return x_dev, y_dev, psi_dev
    ...
    def step(self, u_ev=None, verbose=False):
        ...
        if u_ev is None:
            ...
            self.ev.step(np.array([self.ev.clip_vel_acc(self.ev.traj[:,self.ev.t],self.ev.idm(v_des, dv, ds)),0.]))
        else:
            self.ev.step(u_ev)
        self.t+=1
    ...
    def _get_preds(self, u_opt):
        ...
        for t in range(N):
            ...
            for i in tv_list:
                idx_=set(tv_list+ped_list)-set([i])
    ...
    def draw_intersection(self, ax, i):
        ...
        ax.add_patch(Rectangle((1, -30),30,70,linewidth=1,edgecolor='darkgrey', fc='darkgrey',fill=True, alpha=0.5))
        ...
        ax.plot([1, 1], [-7.5, -30], color='k', lw=2)
        ax.plot([31, 31], [-7.5, -30], color='k', lw=2)
