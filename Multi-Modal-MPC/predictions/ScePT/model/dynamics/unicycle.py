...
class Unicycle(Dynamic):
    ...
    def fast_integrate(self,x,u,dt):
        ...
        if isinstance(x,np.ndarray):
            dx = np.hstack((np.cos(theta)*(x[...,2:3]+u[...,0:1]*dt*0.5),np.sin(theta)*(x[...,2:3]+u[...,0:1]*dt*0.5),u))
        elif isinstance(x,torch.Tensor):
            dx = torch.cat((torch.cos(theta)*(x[...,2:3]+u[...,0:1]*dt*0.5),torch.sin(theta)*(x[...,2:3]+u[...,0:1]*dt*0.5),u),dim=-1)
        else:
